  ...
  function(filename, labels = train_labels) {
    ...
    usertimedom <- 
      ...
      summarise(
        ...
        AR1.2 = cor(X1, lag(X1, n = 3), use = "pairwise"),
        ...
        reg1 = ar(X1, aic=FALSE, order.max=1)$ar,
        reg2 = ar(X2, aic=FALSE, order.max=1)$ar,
        reg3 = ar(X3, aic=FALSE, order.max=1)$ar,
        fft.1 = mean(abs(fft(X1))),
        fft.2 = mean(abs(fft(X2))),
        fft.3 = mean(abs(fft(X3))),
        psd.1 = mean(spectrum(X1, plot = F)$spec),
        psd.2 = mean(spectrum(X2, plot = F)$spec),
...
ar(rnorm(), aic=FALSE, order.max=p)$ar
